...
def active():
    ...
    st.markdown("""
    # 動能選股
    #### 資料來源：[S&P500成份股](https://www.slickcharts.com/sp500)加[Yahoo Finance Screeners](https://finance.yahoo.com/screener)
    ####
    """)
    ...
    if run:
        ...
        watchlist_us = list(set(sid1 + sid2 + sid3 + sid4 + sid5 + sid6 + sid7 + sid8))
    ...
    kbar(ticker, horizon = 'free', mkt = 'us')


    st.markdown('------------------------------------------------------------------------------------')
    col1, col2 = st.beta_columns(2)
    start = col1.date_input('Start', value = pd.to_datetime('2021-04-01'), key='3')
    end = col2.date_input('End', key='4')

    sids = st.radio('同期強弱勢比較',('國際指數','SPDR SECTORS','ARKK Top20')) #'Freddy的MFA', '我瞎湊的MFA'
    if sids == '國際指數':
        df = rebase(['^DJI','^IXIC','^GSPC','^SOX','^GDAXI','EWT','^TWII','^TWOII'])
        df.columns = ['道瓊','那斯達克','標普500','費城半導體','德國DAX','MSCI台灣','加權','櫃買']
    elif sids == 'SPDR SECTORS':
    ...
    elif sids == 'ARKK Top20':
        df = rebase(['TSLA','TDOC','ROKU','SQ','SHOP','ZM','TWLO','COIN','SPOT','Z',
        'U','EXAS','CRSP','PLTR','TXG','DOCU','DKNG','NVTA','TWTR','BEAM'])
    st.line_chart(df.sort_values(by = df.index[-1], ascending= False, axis= 1))


    st.markdown('------------------------------------------------------------------------------------')
    col1, col2 = st.beta_columns(2)
    start = col1.date_input('Start', value = pd.to_datetime('2021-04-01'), key='5')
    end = col2.date_input('End', key='6')

    blue_chips = ['AAPL','MSFT','AMZN','GOOG','FB','TSLA','TSM','BABA','V','JPM','JNJ','WMT','MA','NVDA','UNH','BA']
    weighted = st.multiselect('前16大權值股，挑幾檔順眼的來比一比',blue_chips, default = ['NVDA','AAPL','TSLA']) # 參1字太小不如不放
